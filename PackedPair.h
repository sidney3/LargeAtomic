...
public:
  ...
  constexpr T first() const { return static_cast<T>(data & 0xFFFFFFFF); }
  constexpr U second() const {
    return static_cast<U>((data >> 32) & 0xFFFFFFFF);
  }

  // Ensure the size is fixed at 64 bits.

  // Comparison for equality (useful for CAS operations).
  constexpr bool operator==(const PackedPair &other) const {
